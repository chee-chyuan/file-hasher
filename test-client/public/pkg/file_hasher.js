...
export function generate_row_hash(row_title_js, row_content_js) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.generate_row_hash(retptr, addHeapObject(row_title_js), addHeapObject(row_content_js));
        var r0 = getInt32Memory0()[retptr / 4 + 0];
        var r1 = getInt32Memory0()[retptr / 4 + 1];
        return getStringFromWasm0(r0, r1);
    } finally {
...
* @returns {any}
*/
export function get_file_commitment_and_selected_row(row_title_js, row_content_js, row_selector_js) {
    const ret = wasm.get_file_commitment_and_selected_row(addHeapObject(row_title_js), addHeapObject(row_content_js), addHeapObject(row_selector_js));
...
export function generate_proof(row_title_js, row_content_js, row_selector_js) {
    const ret = wasm.generate_proof(addHeapObject(row_title_js), addHeapObject(row_content_js), addHeapObject(row_selector_js));
    return takeObject(ret);
}
...
* @returns {boolean}
*/
export function verify_correct_selector(accumulator_hash_js, row_accumulator_js, proof_js) {
    const ret = wasm.verify_correct_selector(addHeapObject(accumulator_hash_js), addHeapObject(row_accumulator_js), addHeapObject(proof_js));
    return ret !== 0;
}
